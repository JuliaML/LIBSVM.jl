...
let libsvm = C_NULL
    ...
    function get_libsvm()
        if libsvm == C_NULL
            ccall((:svm_set_print_string_function, "libsvm"), Void,
...
function svmtrain{T, U<:Real}(labels::AbstractVector{T},
    ...
    verbosity = verbose
    ptr = ccall((:svm_train, "libsvm"), Ptr{Void}, (Ptr{SVMProblem},
...
function svmcv{T, U<:Real, V<:Real, X<:Real}(labels::AbstractVector{T},
    ...
    for i = 1:nfolds
        ...
        for j = 1:length(params)
            ...
            correct = 0
            for k in fold_test
                correct += ccall((:svm_predict_values,"libsvm"), Float64, (Ptr{Void},
                    Ptr{SVMNode}, Ptr{Float64}), ptr, nodeptrs[k], decvalues) ==
                    idx[k]
            end
            ccall((:svm_free_model_content,"libsvm"), Void, (Ptr{Void},), ptr)
...
svmfree(model::SVMModel) = ccall((:svm_free_model_content,"libsvm"), Void, (Ptr{Void},),
    model.ptr)
...
function svmpredict{T, U<:Real}(model::SVMModel{T},
    ...
    # this won't work since the (fn,lib) must be a constant
    #fn = model.param[1].probability == 1 ? :svm_predict_probability :   :svm_predict_values
    if model.param[1].probability == 1
        for i = 1:ninstances
            output = ccall((:svm_predict_probability, "libsvm"), Float64, (Ptr{Void}, Ptr{SVMNode}, Ptr{Float64}),
                model.ptr, nodeptrs[i], pointer(decvalues, nlabels*(i-1)+1))
            class[i] = model.labels[round(Int,output)]
        end
    else
        for i = 1:ninstances
            output = ccall((:svm_predict_values, "libsvm"), Float64, (Ptr{Void}, Ptr{SVMNode}, Ptr{Float64}),
                model.ptr, nodeptrs[i], pointer(decvalues, nlabels*(i-1)+1))
            class[i] = model.labels[round(Int,output)]
        end
    end
