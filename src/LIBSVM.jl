...
let libsvm = C_NULL
    ...
    function get_libsvm()
        if libsvm == C_NULL
            @windows_only begin
                libsvm = Libdl.dlopen(joinpath(Pkg.dir(), "LIBSVM", "deps", "libsvm"))
            end
            @linux_only begin
...
function svmpredict{T, U<:Real}(model::SVMModel{T},
    ...
    if model.param[1].probability == 1
        for i = 1:ninstances
            output = ccall(svm_predict_probability(), Float64, (Ptr{Void}, Ptr{SVMNode}, Ptr{Float64}),
                model.ptr, nodeptrs[i], pointer(decvalues, nlabels*(i-1)+1))
            class[i] = model.labels[round(Int,output)]
        end
    else
        for i = 1:ninstances
            output = ccall(svm_predict_values(), Float64, (Ptr{Void}, Ptr{SVMNode}, Ptr{Float64}),
                model.ptr, nodeptrs[i], pointer(decvalues, nlabels*(i-1)+1))
            class[i] = model.labels[round(Int,output)]
        end
    end
